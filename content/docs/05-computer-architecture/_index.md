...
flowchart LR
    A["①<br/>CPU 구조<br/>아키텍처"] --"하드웨어<br/>연산 기반"--> B["②<br/>메모리<br/>계층 구조"]
    B --"속도·용량<br/>최적화"--> C["③<br/>AI 반도체<br/>최신 트렌드"]
    C --"현대<br/>하드웨어"--> D["④<br/>프로세스<br/>스레드 관리"]
    D --"멀티태스킹<br/>기반"--> E["⑤<br/>동기화<br/>교착 상태"]
    E --"자원 충돌<br/>해결"--> F["⑥<br/>메모리<br/>관리"]
    F --"가상화<br/>확장"--> G["⑦<br/>저장장치<br/>파일 시스템"]
