---
title: 애플리케이션 및 개발 보안
weight: 3
---

애플리케이션 및 개발 보안은 소프트웨어 설계·개발·배포 전 단계에서 보안 취약점을 식별하고 제거하기 위한 방법론과 도구를 다루며, OWASP 기반 웹 취약점 분석부터 Secure SDLC·시큐어 코딩·자동화 분석까지 개발 보안의 전 영역을 포괄한다.  
각 토픽은 공격 원리·방어 기법·실무 적용 관점에서 구성되어 정보관리기술사·컴퓨터시스템응용기술사 시험과 실무 보안 개발 모두에 활용할 수 있다.

## 학습 토픽

| 토픽 | 핵심 내용 | 보안 영역 |
|---|---|---|
| **웹 애플리케이션 취약점 (OWASP)** | OWASP Top 10, SQL Injection, XSS, CSRF 공격 원리·탐지·방어 | 웹 보안, 취약점 분석 |
| **소프트웨어 개발 보안 생명주기 (Secure SDLC)** | MS-SDL·Seven Touchpoints·CLASP, 위협 모델링 STRIDE/DREAD | 개발 프로세스 보안 |
| **시큐어 코딩 및 보안 분석 (SAST/DAST/IAST)** | 행안부 7대 취약점, SAST·DAST·IAST·RASP 비교, DevSecOps 통합 | 코드 보안, 자동화 분석 |
